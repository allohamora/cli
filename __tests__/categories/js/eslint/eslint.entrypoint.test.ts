...
describe('eslint', () => {
  test('builds config with plugins and parser options', () => {
    expect(
      buildConfig(
        createConfig({
          eslintConfig: {
  ...
  test('builds typescript config with rules', () => {
    expect(
      buildConfig(
        createConfig({
          eslintConfig: {
  ...
  test('installs default eslint dependencies and writes formatted config', async () => {
    await eslint();
  ...
  test('adds default lint scripts to package.json', async () => {
