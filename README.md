...
| **publish-workflow**      | sets up a github publish workflow that publishes the package to [npm](https://www.npmjs.com) on a workflow dispatch                                                                                                                                                               | `nvmrc`                                            |
| **release-workflow**      | sets up a github release workflow that creates a release from the [conventional-changelog](https://github.com/conventional-changelog/conventional-changelog) CHANGELOG.md from the default branch on workflow dispatch                                                            | `nvmrc`, `publish-workflow`                        |
...
| **check-workflow**        | sets up a github check workflow that runs available lint, format, typecheck, check, and build scripts on push                                                                                                                                                                     | `eslint`, `prettier`, `nvmrc`                      |
...
| **set-pr-title-workflow** | sets up a github workflow that sets the PR title to the branch name on open                                                                                                                                                                                                       |                                                    |
...
| **engines**               | sets up engines and packageManager in package.json with the current node and npm versions                                                                                                                                                                                         |                                                    |
...
| **personal-devcontainer** | creates a `.devcontainer/` setup (Node.js, GitHub CLI, Claude Code, and Codex, config persisted via volumes) for a personal VS Code Dev Container development environment; also adds a privileged Docker-in-Docker feature if a `docker-compose.yml`/`docker-compose.yaml` exists |                                                    |
